...
def main(args):
    ...
    default_mlruns_uri = (Path(project_root) / "mlruns").resolve().as_uri()
    mlruns_path = args.mlflow_uri or default_mlruns_uri  # Local file-based tracking
    ...
    with mlflow.start_run():
        ...
        accuracy = accuracy_score(y_test, y_pred)      # Overall classification accuracy
        ...
        # Also export a local serving bundle used by FastAPI/Docker runtime.
        # This keeps inference independent of MLflow tracking server availability.
        serving_model_dir = os.path.join(artifacts_dir, "model")
        if os.path.exists(serving_model_dir):
            import shutil
            shutil.rmtree(serving_model_dir)
        mlflow.sklearn.save_model(model, path=serving_model_dir)

        feature_txt_path = os.path.join(serving_model_dir, "feature_columns.txt")
        with open(feature_txt_path, "w", encoding="utf-8") as f:
            f.write("\n".join(feature_cols))
        print(f"✅ Exported serving bundle to {serving_model_dir}")
